...
export default function Product(props) {
  ...
  const suggested1SliderOptions = {
    ...
    slides:{
        perView: 4,
        spacing: 30,
        number: product.similarProducts?.length > 0 ? product.similarProducts[0].length : 0 
    },
    breakpoints: {
        '(max-width: 750px)': {
            mode: "snap",
  ...
  const [suggested1SliderRef, suggested1InstanceRef] = useKeenSlider(suggested1SliderOptions,[
    (slider) => {
    let timeout;
  ...
  const suggested2SliderOptions = {
    ...
    slides:{
        perView: 4,
        spacing: 30,
        number: product.similarProducts?.length > 1 ? product.similarProducts[1].length : 0 
    },
    breakpoints: {
        '(max-width: 750px)': {
            mode: "snap",
  ...
  const [suggested2SliderRef, suggested2InstanceRef] = useKeenSlider(suggested2SliderOptions,[
      (slider) => {
      let timeout;
      function clearNextTimeout() {
          clearTimeout(timeout);
  ...
  const ViewImage = (props) => {
    return (
      <div className="viewImageModal" style={{ display: (props.isOpen ? '' : 'none') }}  onClick={() => setIsOpen(false)}>
        <div ref={viewImagesSliderRef} className={`keen-slider ${styles.product_viewer_slider}`}>
  ...
  const onAddToCart = async()=>{
    try{
      ...
      const addToCartResponse = await axios.post('/user/cart',{ products: [{ id: product.id, quantity: 1 }] });
  ...
  return (
    <>
      ...
      <main>
          ...
          <div className="container">
            <div className="product-imgs">
              ...
              <div className="main-img">
                <FavoriteButton className="add-favorite" productId={product.id} liked={product.isFavorite}/>
                <div
            ...
            <div className="product-details">
              <div className="title">
                <div className="box-title">
                  <h5>{i18n.language === "en" ? product.name_en : product.name_ar}</h5>
                </div>
              </div>
              ...
              <div className="shearing">
                ...
                <Link target="_blank" href={"https://wa.me/?text="+encodeURIComponent(props.host+router.asPath)}>
                  <MdWhatsapp size={21}/>
                </Link>
              </div>
        ...
        {
          product.similarProducts?.length > 0 && product.similarProducts[0].length > 0 ?
          <div ref={suggested1SliderRef} className={`${styles.product_slider} keen-slider`}>
            {  product.similarProducts[0].map((suggestedProduct) => (
                <div className="keen-slider__slide" key={"sug_product_"+suggestedProduct.id} >
        ...
        {
          product.similarProducts?.length > 1 && product.similarProducts[1].length > 0 ?
          <div ref={suggested2SliderRef} className={`${styles.product_slider} keen-slider`}>
