        ...
        <section className={styles.container}>
            {
                product_groups.map((product_group, index) => (
                    <Link aria-label={i18n.language === 'en' ? product_group.name_en : product_group.name_ar} key={index} href={`/group/${encodeURIComponent(product_group.id)}`} className={styles.category}>
                        ...
                        <Image loading='eager' className={styles.group_image} quality={50} fill src={product_group.images &&  product_group.images.length > 0 ? ( imageURI + product_group.images[0].link ) : ''} alt={i18n.language === 'en' ? product_group.name_en : product_group.name_ar} sizes='@media (max-width: 395px) 280px, 370px'/>
                    </Link>
                ))
            }
        </section>
