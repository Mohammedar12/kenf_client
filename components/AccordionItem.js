    ...
    return (
        <div className="accordion-item">
            <h2 className="accordion-header" id={`flush-faq-${index}`}>
                <button onClick={()=>{props.setActiveIndex(index)}} className="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                    data-bs-target={`#flush-collapseFaq-${index}`} aria-expanded="false"
                    aria-controls={`flush-collapseFaq-${index}`}>
                    {title}
                </button>
            </h2>
            <div id={`flush-collapseFaq-${index}`} className={`accordion-collapse collapse ${props.activeIndex === index ? 'show' : ''}`}
